...
namespace InterLinkGenius\Data;
...
class Table_Extension {
	 ...
	 * - Extends interlink_genius_links table with PRO columns and indexes
	...
	public static function initialize_schema( $force_reprocess = false ) {
		// Extend interlink_genius_links table with PRO columns.
	...
	/**
	 * Ensure PRO columns exist in interlink_genius_links table.
	 *
	...
	private static function ensure_columns_exist() {
		...
		if ( ! DB::check_table_exists( 'interlink_genius_links' ) ) {
			$schema = "
				...
				KEY `idx_target_blank` (`target_blank`),
				KEY `idx_is_nofollow` (`is_nofollow`),
				KEY `idx_created_at` (`created_at`),
				KEY `idx_post_type` (`post_id`, `type`),
				KEY `idx_type_nofollow` (`type`, `is_nofollow`),
				KEY `idx_post_type_created` (`post_id`, `type`, `created_at`),
				FULLTEXT KEY `idx_search` (`anchor_text`, `url`)
			";
			self::create_table_if_not_exists( 'interlink_genius_links', $schema );
			return true;
		}
	 ...
	 *
	 * This method creates the interlink_genius_audit table if it doesn't exist.
	...
	public static function ensure_status_table_exists() {
		...
		return self::create_table_if_not_exists( 'interlink_genius_audit', $schema );
	}
	...
	public function on_delete_blog( $tables ) {
		...
		$tables[] = $wpdb->prefix . 'interlink_genius_internal_meta';
		$tables[] = $wpdb->prefix . 'interlink_genius_audit';
		$tables[] = $wpdb->prefix . 'interlink_genius_history';
		$tables[] = $wpdb->prefix . 'interlink_genius_snapshots';
		$tables[] = $wpdb->prefix . 'interlink_genius_maps';
		$tables[] = $wpdb->prefix . 'interlink_genius_map_variations';
	...
	public static function ensure_all_pro_tables_exist() {
		...
		// Meta table for link counts.
		if ( self::ensure_internal_meta_table_exists() ) {
			$any_created = true;
		}
	...
	/**
	 * Ensure internal meta table exists.
	 *
	 * @return bool True if table was created, false if it already existed.
	 */
	public static function ensure_internal_meta_table_exists() {
		$schema = "
			...
			KEY `idx_internal` (`internal_link_count`),
			KEY `idx_external` (`external_link_count`),
			KEY `idx_incoming` (`incoming_link_count`)
		";
		return self::create_table_if_not_exists( 'interlink_genius_internal_meta', $schema );
	}

	/**
	 * Update internal meta counts for a specific post.
	 *
	 * @param int $post_id Post ID.
	 */
	public static function update_internal_meta_counts( $post_id ) {
		global $wpdb;
		$links_table = $wpdb->prefix . 'interlink_genius_links';
		$meta_table  = $wpdb->prefix . 'interlink_genius_internal_meta';

		self::ensure_internal_meta_table_exists();

		// Get outgoing counts for this post.
		$internal_cnt = (int) $wpdb->get_var( $wpdb->prepare(
			"SELECT COUNT(*) FROM {$links_table} WHERE post_id = %d AND type = 'internal'",
			$post_id
		) );
		$external_cnt = (int) $wpdb->get_var( $wpdb->prepare(
			"SELECT COUNT(*) FROM {$links_table} WHERE post_id = %d AND type = 'external'",
			$post_id
		) );

		// Insert or update outgoing counts.
		$wpdb->query( $wpdb->prepare(
			"INSERT INTO {$meta_table} (object_id, internal_link_count, external_link_count, incoming_link_count)
			VALUES (%d, %d, %d, 0)
			ON DUPLICATE KEY UPDATE internal_link_count = %d, external_link_count = %d",
			$post_id, $internal_cnt, $external_cnt, $internal_cnt, $external_cnt
		) );

		// Update incoming count for target posts of this post.
		$wpdb->query(
			"UPDATE {$meta_table} m
			LEFT JOIN (
			...
			) l ON m.object_id = l.target_post_id
			SET m.incoming_link_count = COALESCE(l.cnt, 0)"
		);
	}

	/**
	 * Rebuild all internal meta counts.
	 */
	public static function rebuild_all_internal_meta() {
		global $wpdb;
		$meta_table  = $wpdb->prefix . 'interlink_genius_internal_meta';
		$links_table = $wpdb->prefix . 'interlink_genius_links';
		...
		// Clear the meta table.
		$wpdb->query( "TRUNCATE TABLE {$meta_table}" );

		// Insert all published posts.
		$wpdb->query(
			"INSERT INTO {$meta_table} (object_id, internal_link_count, external_link_count, incoming_link_count)
			SELECT ID, 0, 0, 0
			FROM {$wpdb->posts}
		...
		// Update outgoing internal counts.
		$wpdb->query(
			"UPDATE {$meta_table} m
			INNER JOIN (
				...
				WHERE type = 'internal'
				GROUP BY post_id
			) l ON m.object_id = l.post_id
			SET m.internal_link_count = l.cnt"
		);

		// Update outgoing external counts.
		$wpdb->query(
			"UPDATE {$meta_table} m
			INNER JOIN (
				SELECT post_id, COUNT(*) as cnt
			...
			SET m.external_link_count = l.cnt"
		);

		// Update incoming internal counts.
		$wpdb->query(
			"UPDATE {$meta_table} m
			INNER JOIN (
				SELECT target_post_id, COUNT(*) as cnt
			...
			SET m.incoming_link_count = l.cnt"
		);
	}
	...
	private static function ensure_bulk_update_history_table_exists() {
		...
		return self::create_table_if_not_exists( 'interlink_genius_history', $schema );
	}
	...
	private static function ensure_bulk_update_snapshots_table_exists() {
		...
		return self::create_table_if_not_exists( 'interlink_genius_snapshots', $schema );
	}
	...
	private static function ensure_keyword_maps_table_exists() {
		...
		return self::create_table_if_not_exists( 'interlink_genius_maps', $schema );
	}
	...
	private static function ensure_keyword_variations_table_exists() {
		...
		$created = self::create_table_if_not_exists( 'interlink_genius_map_variations', $schema );

		// Add foreign key constraint if table was just created and parent table exists.
		if ( $created && DB::check_table_exists( 'interlink_genius_maps' ) ) {
			global $wpdb;
			$table           = $wpdb->prefix . 'interlink_genius_map_variations';
			$maps_table      = $wpdb->prefix . 'interlink_genius_maps';
