...
class Query_Builder {
	...
	public static function get_posts( $args = [] ) {
		...
		$results = $wpdb->get_results(
			"SELECT p.ID as post_id,
			 ...
			 FROM {$wpdb->prefix}interlink_genius_internal_meta m
			 ...
			 LEFT JOIN {$wpdb->postmeta} pm_keyword ON p.ID = pm_keyword.post_id AND pm_keyword.meta_key = 'interlink_genius_focus_keyword'
	...
	public static function get_posts_count( $args = [] ) {
		...
		// Build JOIN clause - start from interlink_genius_internal_meta and join posts.
		...
		if ( ! empty( $args['seo_score_range'] ) ) {
			...
			$joins .= " LEFT JOIN {$wpdb->postmeta} pm_keyword ON p.ID = pm_keyword.post_id AND pm_keyword.meta_key = 'interlink_genius_focus_keyword'";
		}
	 ...
	 *
	 * Returns true if the interlink_genius_audit table has any entries.
	 *
	...
	public static function has_audit_run_before() {
		...
		if ( ! \InterLinkGenius\Compatibility\Helpers\DB::check_table_exists( 'interlink_genius_audit' ) ) {
			return false;
		}
	...
	private static function get_cache_ttl() {
		 ...
		 */
		return (int) apply_filters( 'interlink_genius/cache_ttl', 30 * MINUTE_IN_SECONDS );
	}
