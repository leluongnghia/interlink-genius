	...
	return (
		<>
			<BlockControls>
				<ToolbarButton
					icon="update"
					label={ __( 'Refresh related posts', 'interlink-genius' ) }
					onClick={ () => doRefresh() }
					disabled={ refreshing }
				/>
			</BlockControls>
			<InspectorControls>
				<PanelBody title={ __( 'UX', 'interlink-genius' ) } initialOpen={ true }>
					<RangeControl
						label={ __( 'Number of posts', 'interlink-genius' ) }
					...
					/>
					<ToggleControl
						label={ __( 'Show image', 'interlink-genius' ) }
						checked={ attributes.showImage }
						onChange={ ( value ) => setAttributes( { showImage: value } ) }
					/>
					{
						attributes.showImage && <SelectControl
							label={ __( 'Image size', 'interlink-genius' ) }
							value={ attributes.imageSize }
							options={ [
								{ value: 'thumbnail', label: __( 'Thumbnail', 'interlink-genius' ) },
								{ value: 'medium', label: __( 'Medium', 'interlink-genius' ) },
								{ value: 'large', label: __( 'Large', 'interlink-genius' ) },
							] }
							onChange={ ( value ) => setAttributes( { imageSize: value } ) }
						/>
					}
					<ToggleControl
						label={ __( 'Show excerpt', 'interlink-genius' ) }
						checked={ attributes.showExcerpt }
						onChange={ ( value ) => setAttributes( { showExcerpt: value } ) }
					/>
					<ToggleControl
						label={ __( 'Show date', 'interlink-genius' ) }
						checked={ attributes.showDate }
						onChange={ ( value ) => setAttributes( { showDate: value } ) }
					/>
					<ToggleControl
						label={ __( 'Show taxonomy chips', 'interlink-genius' ) }
						checked={ attributes.showTerms }
						onChange={ ( value ) => setAttributes( { showTerms: value } ) }
					/>
				</PanelBody>
				<PanelBody title={ __( 'Texts', 'interlink-genius' ) } initialOpen={ false }>
					<TextControl
						label={ __( 'Title', 'interlink-genius' ) }
						value={ attributes.title }
						onChange={ ( value ) => setAttributes( { title: value } ) }
					/>
					<TextControl
						label={ __( 'Button text', 'interlink-genius' ) }
						value={ attributes.buttonText }
						onChange={ ( value ) => setAttributes( { buttonText: value } ) }
					/>
					<TextControl
						label={ __( 'Button URL', 'interlink-genius' ) }
				...
				{ fetchError ? ( () => {
						...
						<div>
							...
							{ ! hasContentAIIssue && (
								<Notice status="error" isDismissible={ false }>
									{ fetchError?.message || __( 'An error occurred while fetching related posts.', 'interlink-genius' ) }
								</Notice>
							) }
						</div>
					)
					...
					<ServerSideRender
						...
						ErrorResponsePlaceholder={ ( { error } ) => {
							...
							return (
								<Notice status="error" isDismissible={ false }>
									<strong>{ __( 'Failed to load related posts.', 'interlink-genius' ) }</strong>
									<p>{ error?.message || __( 'An unknown error occurred. Please try refreshing.', 'interlink-genius' ) }</p>
									<button
										className="button button-primary"
										onClick={ () => doRefresh() }
										disabled={ refreshing }
									>
										{ refreshing ? __( 'Refreshing…', 'interlink-genius' ) : __( 'Retry', 'interlink-genius' ) }
									</button>
								</Notice>
							)
						} }
