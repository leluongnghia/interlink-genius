...
use InterLinkGenius\Background\Process_Coordinator;
use InterLinkGenius\Background\Processor_Utils;
...
class Processor extends \WP_Background_Process {
	...
	use \InterLinkGenius\Background\Background_Processor_Error_Handler;
	use \InterLinkGenius\Background\Progress_Helper;
	...
	public static function get_chunk_size() {
		return Processor_Utils::get_filtered_chunk_size(
			'interlink_genius/bulk_update_chunk_size',
			10
		);
	}
	...
	public function start( $filters, $update_config, $link_ids = null, $is_rollback = false, $rollback_batch_id = null ) {
		...
		return [
			...
			'total_posts' => $total_posts,
			'total_links' => $total_links,
			'message'     => __( 'Bulk update scheduled. Processing in background.', 'interlink-genius' ),
		];
	}
	...
	private function queue_affected_links_for_crawler( $post_ids ) {
		...
		$links = DB::get_results(
			$wpdb->prepare(
				...
				FROM {$wpdb->prefix}interlink_genius_links
		...
		if ( ! empty( $links ) ) {
			...
			\InterLinkGenius\Background\Link_Status_Crawler::queue_links_batch( $links );
		}
	}
