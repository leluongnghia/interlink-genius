...
class Keyword_Map_Processor extends \WP_Background_Process {
	...
	public static function get_chunk_size() {
		return Processor_Utils::get_filtered_chunk_size(
			'interlink_genius/keyword_map_chunk_size',
			10
		);
	}
	...
	public function start( $keyword_map, $selected_items = null, $preview_results = null, $is_rollback = false, $rollback_batch_id = null ) {
		...
		if ( $is_rollback && $rollback_batch_id ) {
			...
			if ( empty( $preview_results ) || ! is_array( $preview_results ) ) {
				return new WP_Error(
					'no_preview_data',
					__( 'Preview data is required to execute keyword map.', 'interlink-genius' ),
					[ 'status' => 400 ]
				);
			}
			...
			if ( empty( $all_changes ) ) {
				return new WP_Error(
					'no_changes_found',
					__( 'No changes found to apply.', 'interlink-genius' )
				);
			}
		...
		$message = $is_rollback
			...
			: __( 'Keyword map execution started. Processing in background.', 'interlink-genius' );
	...
	protected function task( $item ) {
		...
		if ( $is_rollback && $rollback_batch_id ) {
			...
			$snapshot = $wpdb->get_row(
				$wpdb->prepare(
					"SELECT original_content, link_changes FROM {$wpdb->prefix}interlink_genius_snapshots WHERE batch_id = %s AND post_id = %d",
					$rollback_batch_id,
					$post_id
				)
			);
