...
class Storage {
	...
	public function get_keyword_map( $id ) {
		...
		$result = $wpdb->get_row(
			$wpdb->prepare(
				"SELECT * FROM {$wpdb->prefix}interlink_genius_maps WHERE id = %d",
				$id
			)
		);
	...
	public function get_keyword_maps( $args = [] ) {
		...
		$results = $wpdb->get_results(
			"SELECT * FROM {$wpdb->prefix}interlink_genius_maps {$where} {$orderby_sql} {$limit_sql}"
		);
	...
	public function save_keyword_map( $data ) {
		...
		$table = $wpdb->prefix . 'interlink_genius_maps';
	...
	public function delete_keyword_map( $id ) {
		...
		$result = $wpdb->delete(
			$wpdb->prefix . 'interlink_genius_maps',
			[ 'id' => $id ],
			[ '%d' ]
		);
	...
	public function bulk_delete_keyword_maps( $ids ) {
		...
		$delete_sql   = "DELETE FROM {$wpdb->prefix}interlink_genius_maps WHERE id IN ($placeholders)";
	...
	public function bulk_toggle_keyword_maps( $ids, $enabled ) {
		...
		$ids          = array_map( 'absint', $ids );
		$placeholders = implode( ',', array_fill( 0, count( $ids ), '%d' ) );
		$update_sql   = "UPDATE {$wpdb->prefix}interlink_genius_maps
	...
	public function count_keyword_maps( $args = [] ) {
		...
		$where = $this->build_keyword_maps_where( $args );
		$count = $wpdb->get_var(
			"SELECT COUNT(*) FROM {$wpdb->prefix}interlink_genius_maps {$where}"
		);
	...
	public function update_execution_stats( $id ) {
		...
		$result = $wpdb->query(
			$wpdb->prepare(
				"UPDATE {$wpdb->prefix}interlink_genius_maps
	...
	public function get_execution_stats( $id ) {
		...
		$executions = $wpdb->get_var(
			$wpdb->prepare(
				"SELECT COUNT(*)
				FROM {$wpdb->prefix}interlink_genius_history
	...
	public function get_variations( $keyword_map_id ) {
		...
		$results = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT * FROM {$wpdb->prefix}interlink_genius_map_variations
	...
	public function get_variations_batch( $keyword_map_ids ) {
		...
		$query           = "SELECT * FROM {$wpdb->prefix}interlink_genius_map_variations
	...
	public function add_variation( $keyword_map_id, $variation, $source = 'manual' ) {
		...
		$result = $wpdb->insert(
			$wpdb->prefix . 'interlink_genius_map_variations',
			[
	...
	public function count_variations( $keyword_map_id, $source = null ) {
		...
		if ( $source ) {
			$count = $wpdb->get_var(
				$wpdb->prepare(
					"SELECT COUNT(*) FROM {$wpdb->prefix}interlink_genius_map_variations
	...
	public function variation_exists( $keyword_map_id, $variation ) {
		...
		$exists = $wpdb->get_var(
			$wpdb->prepare(
				"SELECT id FROM {$wpdb->prefix}interlink_genius_map_variations
	...
	public function find_duplicate_name( $name, $exclude_id = null ) {
		...
		if ( $exclude_id ) {
			return $wpdb->get_row(
				$wpdb->prepare(
					"SELECT id, name FROM {$wpdb->prefix}interlink_genius_maps WHERE LOWER(name) = LOWER(%s) AND id != %d LIMIT 1",
					$name,
					$exclude_id
				)
			);
		...
		return $wpdb->get_row(
			$wpdb->prepare(
				"SELECT id, name FROM {$wpdb->prefix}interlink_genius_maps WHERE LOWER(name) = LOWER(%s) LIMIT 1",
				$name
			)
		);
	}
	...
	public function find_duplicate_keyword( $keyword, $exclude_id = null ) {
		...
		if ( $exclude_id ) {
			$existing_map = $wpdb->get_row(
				$wpdb->prepare(
					"SELECT id, name FROM {$wpdb->prefix}interlink_genius_maps
		...
		if ( $exclude_id ) {
			$existing_variation = $wpdb->get_row(
				$wpdb->prepare(
					...
					FROM {$wpdb->prefix}interlink_genius_map_variations v
					JOIN {$wpdb->prefix}interlink_genius_maps m ON v.keyword_map_id = m.id
