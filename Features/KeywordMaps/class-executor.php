...
use InterLinkGenius\Features\KeywordMaps\Utils\Link_Applier;
...
class Executor {
	...
	private function build_exclusion_sql( $global_settings ) {
		...
		// 1. Exclude posts with interlink_genius_auto_linking_disabled meta.
		$exclusion_clauses[] = "NOT EXISTS (
			SELECT 1 FROM {$wpdb->postmeta} pm
			WHERE pm.post_id = p.ID
			AND pm.meta_key = 'interlink_genius_auto_linking_disabled'
			AND pm.meta_value = '1'
		)";
	...
	public function preview( $keyword_map ) {
		...
		if ( empty( $variations ) ) {
			return new \WP_Error(
				'no_keywords',
				__( 'No keywords or variations defined for this keyword map.', 'interlink-genius' )
			);
		}
		...
		if ( empty( $posts ) ) {
			return [
				...
				'sample_changes' => [],
				'warnings'       => [],
				'message'        => __( 'No posts found containing the specified keywords.', 'interlink-genius' ),
			];
		}
		...
		if ( 0 === $total_links ) {
			return [
				...
				'sample_changes' => [],
				'warnings'       => [],
				'message'        => __( 'No new link opportunities found. All keyword matches are already linked or in content that cannot be modified.', 'interlink-genius' ),
			];
		}
