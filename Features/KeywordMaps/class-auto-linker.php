...
class Auto_Linker {
	...
	private function __construct() {
		...
		$this->action( 'interlink_genius_keyword_maps_auto_link', 'process_auto_link' );
	}
	...
	public function handle_save_post( $post_id, $post ) {
		...
		wp_schedule_single_event( time() + 10, 'interlink_genius_keyword_maps_auto_link', [ $post_id ] );
	}
	...
	private function execute_maps_for_post( $post, $keyword_maps, $map_variations ) {
		...
		if ( $total_links > 0 ) {
			$post_title = $post->post_title ? $post->post_title : __( '(no title)', 'interlink-genius' );
	 ...
	 * - Post-level opt-out meta (interlink_genius_auto_linking_disabled)
	 *
	...
	private function should_process_post( $post_id, $post ) {
		...
		if ( get_post_meta( $post_id, 'interlink_genius_auto_linking_disabled', true ) ) {
			return false;
		}
