 ...
 *
 * @package    RankMathPro
 * @subpackage InterLinkGenius\Keyword_Maps
 */
...
use InterLinkGenius\Compatibility\Rest_Helper;
use InterLinkGenius\Api\Base_Operation_Controller;
use InterLinkGenius\Services\History_Service;
use InterLinkGenius\Features\KeywordMaps\Utils\Variation_Expander;
...
class Rest extends Base_Operation_Controller {
	...
	public function __construct() {
		$this->namespace = 'interlink-genius/v1';
	...
	public function register_routes() {
		...
		$this->register_preview_endpoints(
			...
			[
				...
				'results_args'     => [
					'keyword_map_id' => [
						'description' => esc_html__( 'Keyword map ID.', 'interlink-genius' ),
						'type'        => 'integer',
						'required'    => true,
					],
				],
		...
		register_rest_route(
			...
			[
				[
					...
					'args'                => [
						'page'     => [
							'description' => esc_html__( 'Page number.', 'interlink-genius' ),
							'type'        => 'integer',
							'default'     => 1,
							'minimum'     => 1,
						],
						'per_page' => [
							'description' => esc_html__( 'Number of items per page.', 'interlink-genius' ),
							'type'        => 'integer',
							'default'     => 20,
							'minimum'     => 1,
							'maximum'     => 100,
						...
						'orderby'  => [
							'description' => esc_html__( 'Column to order by.', 'interlink-genius' ),
							'type'        => 'string',
							'default'     => 'created_at',
							'enum'        => [ 'id', 'name', 'created_at', 'updated_at', 'last_executed_at', 'execution_count' ],
						],
						'order'    => [
							'description' => esc_html__( 'Order direction.', 'interlink-genius' ),
							'type'        => 'string',
							'default'     => 'DESC',
							'enum'        => [ 'ASC', 'DESC' ],
						],
						'enabled'  => [
							'description' => esc_html__( 'Filter by enabled status.', 'interlink-genius' ),
							'type'        => 'boolean',
						],
						'search'   => [
							'description' => esc_html__( 'Search term.', 'interlink-genius' ),
							'type'        => 'string',
						],
					],
				],
		...
		register_rest_route(
			...
			[
				...
				'args'                => [
					'ids'     => [
						'description' => esc_html__( 'Array of keyword map IDs.', 'interlink-genius' ),
						'type'        => 'array',
						'required'    => true,
						'items'       => [ 'type' => 'integer' ],
					],
					'enabled' => [
						'description' => esc_html__( 'New enabled status.', 'interlink-genius' ),
						'type'        => 'boolean',
						'required'    => true,
					],
				],
		...
		register_rest_route(
			...
			[
				...
				[
					...
					'args'                => [
						'variation' => [
							'description' => esc_html__( 'Variation text.', 'interlink-genius' ),
							'type'        => 'string',
							'required'    => true,
						],
						'source'    => [
							'description' => esc_html__( 'Variation source.', 'interlink-genius' ),
							'type'        => 'string',
							'default'     => 'manual',
							'enum'        => [ 'manual', 'ai' ],
						],
		...
		register_rest_route(
			...
			[
				...
				'args'                => [
					'keyword' => [
						'description' => esc_html__( 'Keyword to generate variations for.', 'interlink-genius' ),
						'type'        => 'string',
						'required'    => true,
					],
					'context' => [
						'description' => esc_html__( 'Optional context to help generate better variations.', 'interlink-genius' ),
						'type'        => 'string',
					],
				],
			]
		...
		register_rest_route(
			...
			[
				...
				'args'                => [
					'variations' => [
						'description' => esc_html__( 'Array of variations to accept.', 'interlink-genius' ),
						'type'        => 'array',
						'required'    => true,
					],
				],
		...
		// GET /ai-settings - Lấy cài đặt nhà cung cấp AI.
		register_rest_route(
			$this->namespace,
			'/ai-settings',
			[
				...
				[
					...
					'args'                => [
						'provider' => [
							'description' => esc_html__( 'AI provider (gemini, openai, grok).', 'interlink-genius' ),
							'type'        => 'string',
							'required'    => true,
							'enum'        => [ 'gemini', 'openai', 'grok' ],
						],
						'model'    => [
							'description' => esc_html__( 'Model name.', 'interlink-genius' ),
							'type'        => 'string',
							'required'    => true,
						],
						'api_key'  => [
							'description' => esc_html__( 'API Key của nhà cung cấp AI.', 'interlink-genius' ),
							'type'        => 'string',
							'required'    => true,
						],
					],
	...
	private function get_keyword_map_args( $required = true ) {
		return [
			'name'                 => [
				'description' => esc_html__( 'Keyword map name.', 'interlink-genius' ),
				'type'        => 'string',
				'required'    => $required,
			],
			'description'          => [
				'description' => esc_html__( 'Keyword map description.', 'interlink-genius' ),
				'type'        => 'string',
			],
			'target_url'           => [
				'description' => esc_html__( 'Target URL.', 'interlink-genius' ),
				'type'        => 'string',
				'required'    => $required,
			],
			'is_enabled'           => [
				'description' => esc_html__( 'Whether the keyword map is enabled.', 'interlink-genius' ),
				'type'        => 'boolean',
				'default'     => true,
			],
			'max_links_per_post'   => [
				'description' => esc_html__( 'Maximum links per post.', 'interlink-genius' ),
				'type'        => 'integer',
				'default'     => 3,
				'minimum'     => 1,
			],
			'auto_link_on_publish' => [
				'description' => esc_html__( 'Enable auto-linking on publish.', 'interlink-genius' ),
				'type'        => 'boolean',
				'default'     => false,
			],
			'case_sensitive'       => [
				'description' => esc_html__( 'Enable case-sensitive keyword matching.', 'interlink-genius' ),
				'type'        => 'boolean',
				'default'     => false,
			],
		];
	...
	public function get_keyword_map( $request ) {
		...
		if ( ! $map ) {
			return new WP_Error(
				'not_found',
				__( 'Keyword map not found.', 'interlink-genius' ),
				[ 'status' => 404 ]
			);
		}
	...
	public function create_keyword_map( $request ) {
		...
		if ( ! $id ) {
			return new WP_Error(
				'create_failed',
				__( 'Failed to create keyword map.', 'interlink-genius' ),
				[ 'status' => 500 ]
			);
		}
		...
		$message = $this->build_success_message(
			__( 'Keyword map created successfully.', 'interlink-genius' ),
			$skipped_variations
		);
	...
	public function update_keyword_map( $request ) {
		...
		if ( ! $result ) {
			return new WP_Error(
				'update_failed',
				__( 'Failed to update keyword map.', 'interlink-genius' ),
				[ 'status' => 500 ]
			);
		}
		...
		return new WP_REST_Response(
			[
				...
				'message' => __( 'Keyword map updated successfully.', 'interlink-genius' ),
			],
			200
		);
	}
	...
	private function validate_keyword_map_name( $name, $exclude_id = null ) {
		...
		if ( $duplicate_name ) {
			return new WP_Error(
				'duplicate_name',
				__( 'A keyword map with this name already exists. Please use a different name.', 'interlink-genius' ),
				[ 'status' => 400 ]
			);
		}
		...
		if ( $duplicate_keyword ) {
			return new WP_Error(
				...
				sprintf(
					...
					__( 'This keyword already exists in the keyword map "%s". Consider adding your variations there instead.', 'interlink-genius' ),
	...
	private function build_success_message( $base_message, $skipped_variations ) {
		...
		return sprintf(
			...
			__( '%1$s The following variations were skipped because they already exist in other keyword maps: %2$s', 'interlink-genius' ),
			$base_message,
	...
	public function delete_keyword_map( $request ) {
		...
		if ( ! $result ) {
			return new WP_Error(
				'delete_failed',
				__( 'Failed to delete keyword map.', 'interlink-genius' ),
				[ 'status' => 500 ]
			);
		}
		...
		return new WP_REST_Response(
			[
				'success' => true,
				'message' => __( 'Keyword map deleted successfully.', 'interlink-genius' ),
			],
			200
		);
	}
		...
		return new WP_REST_Response(
			[
				...
				'message' => sprintf(
					...
					__( '%d keyword maps updated.', 'interlink-genius' ),
					$updated
				),
			],
			200
		...
		return new WP_REST_Response(
			[
				...
				'message' => sprintf(
					...
					__( '%d keyword maps deleted.', 'interlink-genius' ),
					$deleted
				),
			],
			200
	...
	public function execute_keyword_map( $request ) {
		...
		if ( ! $is_rollback && ! $map->is_enabled ) {
			return new WP_Error(
				'disabled',
				__( 'This keyword map is disabled. Enable it first to execute.', 'interlink-genius' ),
				[ 'status' => 400 ]
			);
		}
	...
	public function preview_keyword_map( $request ) {
		...
		if ( ! $map->is_enabled ) {
			return new WP_Error(
				'disabled',
				__( 'This keyword map is disabled. Enable it first to preview.', 'interlink-genius' ),
				[ 'status' => 400 ]
			);
		}
	...
	public function add_variation( $request ) {
		...
		if ( $this->storage->variation_exists( $id, $variation ) ) {
			return new WP_Error(
				'duplicate_variation',
				__( 'This variation already exists in this keyword map.', 'interlink-genius' ),
				[ 'status' => 400 ]
			);
		}
		...
		if ( $duplicate_keyword ) {
			return new WP_Error(
				...
				sprintf(
					/* translators: %s: existing keyword map name */
					__( 'This keyword already exists in the keyword map "%s".', 'interlink-genius' ),
					$duplicate_keyword['keyword_map']->name
				),
				[
					'status'          => 400,
		...
		if ( ! $variation_id ) {
			return new WP_Error(
				'add_failed',
				__( 'Failed to add variation.', 'interlink-genius' ),
				[ 'status' => 500 ]
			);
		}
		...
		return new WP_REST_Response(
			[
				...
				'variations'   => $variations,
				'message'      => __( 'Variation added successfully.', 'interlink-genius' ),
			],
			201
		);
	}
	...
	public function delete_variation( $request ) {
		...
		if ( ! $result ) {
			return new WP_Error(
				'delete_failed',
				__( 'Failed to delete variation.', 'interlink-genius' ),
				[ 'status' => 500 ]
			);
		}
		...
		return new WP_REST_Response(
			[
				'success' => true,
				'message' => __( 'Variation deleted successfully.', 'interlink-genius' ),
			],
			200
		);
	}
	...
	public function generate_variations( $request ) {
		// Kiểm tra AI client đã cấu hình API key chưa.
		$ai_client = new AI_Client();
		if ( ! $ai_client->is_available() ) {
			return new WP_Error(
				'no_api_key',
				esc_html__( 'Chưa cấu hình API Key AI. Vui lòng vào Cài đặt AI để thiết lập nhà cung cấp.', 'interlink-genius' ),
				[ 'status' => 400 ]
			);
		}
		...
		$total_suggestions = 0;
		$category_labels   = [
			'synonyms'             => __( 'Synonyms', 'interlink-genius' ),
			'related_phrases'      => __( 'Related Phrases', 'interlink-genius' ),
			'long_tail_variations' => __( 'Long-tail Variations', 'interlink-genius' ),
			'common_misspellings'  => __( 'Common Misspellings', 'interlink-genius' ),
		];
		...
		return new WP_REST_Response(
			[
				...
				'keyword'     => $keyword,
				'message'     => __( 'AI variations generated. Review and accept the ones you want to add.', 'interlink-genius' ),
			],
			200
		);
	}
	...
	public function generate_variations_by_keyword( $request ) {
		...
		if ( empty( $keyword ) ) {
			return new WP_Error(
				'missing_keyword',
				__( 'Keyword is required.', 'interlink-genius' ),
				[ 'status' => 400 ]
			);
		}
	...
	public function accept_variations( $request ) {
		...
		if ( empty( $variations ) || ! is_array( $variations ) ) {
			return new WP_Error(
				'invalid_variations',
				__( 'No variations provided.', 'interlink-genius' ),
				[ 'status' => 400 ]
			);
		}
		...
		return new WP_REST_Response(
			[
				...
				'message'    => sprintf(
					...
					__( '%d variations added.', 'interlink-genius' ),
					$added
				),
			],
			200
	...
	public function preview( $request ) {
		return new WP_Error(
			'not_implemented',
			__( 'Use preview_keyword_map endpoint instead.', 'interlink-genius' ),
			[ 'status' => 400 ]
		);
	}
	...
	public function delete_history( $request ) {
		return new WP_Error(
			'not_implemented',
			__( 'Keyword maps do not support history deletion.', 'interlink-genius' ),
			[ 'status' => 400 ]
		);
	}
	/**
	 * Lấy cài đặt nhà cung cấp AI hiện tại.
	 *
	 * @param WP_REST_Request $request Request object.
	...
	public function get_ai_settings( $request ) {
		...
		// Che api_key - chỉ trả về 4 ký tự cuối để UI biết đã cài đặt.
		$masked_key = '';
		if ( ! empty( $settings['api_key'] ) ) {
			$masked_key = str_repeat( '•', max( 0, strlen( $settings['api_key'] ) - 4 ) )
		...
		return new WP_REST_Response(
			[
				...
				'settings'  => [
					...
					'api_key_is_set'    => ! empty( $settings['api_key'] ),
				],
				'providers' => $providers,
			],
			200
	...
	public function save_ai_settings( $request ) {
		...
		if ( ! $saved ) {
			return new WP_Error(
				'save_failed',
				__( 'Không thể lưu cài đặt AI.', 'interlink-genius' ),
				[ 'status' => 500 ]
			);
		}
		...
		return new WP_REST_Response(
			[
				...
				'message' => sprintf(
					...
					__( 'Đã lưu cài đặt AI. Nhà cung cấp: %s, Model: %s.', 'interlink-genius' ),
					AI_Client::PROVIDERS[ $provider ]['label'] ?? $provider,
					$model
				),
			],
			200
