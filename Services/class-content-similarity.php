...
use InterLinkGenius\Compatibility\Singular;
...
class Content_Similarity {
	...
	private function build_post_summaries_batch( $post_ids, $prefetched_posts = [], $prefetched_seo = [] ) {
		...
		$keywords_meta = Batch_Helper::batch_fetch_postmeta( $post_ids, 'interlink_genius_focus_keyword' );
	...
	private function ai_refine_generic( $post_id, $candidates, $limit, $args ) {
		...
		if ( empty( $connect_data ) || empty( $connect_data['username'] ) || empty( $connect_data['api_key'] ) ) {
			return new \WP_Error(
				...
				__( 'Rank Math registration data is missing. Please connect your site to use AI features.', 'interlink-genius' ),
				[ 'status' => 401 ]
			);
		}
		...
		$payload = [
			...
			'plugin_version' => INTERLINK_GENIUS_VERSION,
