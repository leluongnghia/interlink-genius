 ...
 * @since      1.0.264
 * @package    RankMathPro
 * @subpackage InterLinkGenius\Services
...
class History_Service {
	...
	public function get_history( $source_type = null, $page = 1, $per_page = 20, $orderby = 'created_at', $order = 'DESC' ) {
		...
		if ( $source_type ) {
			...
			if ( 'keyword_map' === $source_type ) {
				$join = "LEFT JOIN {$wpdb->prefix}interlink_genius_maps km ON h.keyword_map_id = km.id";
			}
		}
		...
		$history = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT {$select}
				FROM {$wpdb->prefix}interlink_genius_history h
				{$join}
				{$where}
	...
	public function delete_history( $batch_id ) {
		...
		$exists = $wpdb->get_var(
			$wpdb->prepare(
				"SELECT id FROM {$wpdb->prefix}interlink_genius_history WHERE batch_id = %s",
				$batch_id
			)
		);
		...
		if ( ! $exists ) {
			return new WP_Error(
				'history_not_found',
				__( 'History record not found.', 'interlink-genius' ),
				[ 'status' => 404 ]
			);
		}
		...
		if ( false === $deleted ) {
			return new WP_Error(
				'delete_failed',
				__( 'Failed to delete history record.', 'interlink-genius' ),
				[ 'status' => 500 ]
			);
		}
		...
		return [
			'success' => true,
			'message' => __( 'History record deleted successfully.', 'interlink-genius' ),
		];
	}
	...
	public function bulk_delete( $batch_ids ) {
		if ( empty( $batch_ids ) || ! is_array( $batch_ids ) ) {
			return [
				'success' => false,
				'deleted' => 0,
				'message' => __( 'No batch IDs provided.', 'interlink-genius' ),
			];
		}
		...
		return [
			...
			'message' => sprintf(
				...
				__( '%d history records deleted.', 'interlink-genius' ),
				$deleted_count
			),
		];
	}
	...
	private function build_summary( $item, $changes_summary ) {
		switch ( $item->source_type ) {
			case 'bulk_update':
				...
				if ( $from && $to ) {
					return sprintf(
						...
						__( 'Changed <span class="rank-math-history-from">"%1$s"</span> to <span class="rank-math-history-to">"%2$s"</span> in <span class="rank-math-history-count">%3$d</span> links', 'interlink-genius' ),
						$from,
						$to,
				...
				return sprintf(
					...
					__( 'Updated %d links', 'interlink-genius' ),
					(int) $item->affected_links_count
				);
			...
			case 'keyword_map':
				...
				if ( $keyword ) {
					return sprintf(
						...
						__( 'Added %2$d links for keyword <span class="rank-math-history-to">"%1$s"</span>', 'interlink-genius' ),
						$keyword,
						(int) $item->affected_links_count
					);
				}
				...
				return sprintf(
					/* translators: %d: number of links */
					__( 'Added %d links', 'interlink-genius' ),
					(int) $item->affected_links_count
				);
			...
			default:
				return sprintf(
					/* translators: %d: number of links */
					__( 'Affected %d links', 'interlink-genius' ),
					(int) $item->affected_links_count
				);
		}
	}
