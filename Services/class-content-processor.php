...
use InterLinkGenius\Compatibility\Classifier;
use InterLinkGenius\Compatibility\Hooker;
use InterLinkGenius\Compatibility\Helper;
use InterLinkGenius\Compatibility\Helpers\Str;
use InterLinkGenius\Compatibility\Helpers\Url;
...
class Content_Processor {
	...
	public function __construct() {
		...
		$this->filter( 'interlink_genius/links/extract', 'extract_enhanced', 10, 3 );

		// Hook into FREE plugin's save filter to save enhanced data.
		$this->filter( 'interlink_genius/links/save_links', 'save_enhanced_links', 10, 3 );
	}
	...
	public function save_enhanced_links( $override, $post_id, $links ) {
		...
		// Delete existing links for this post to prevent duplicates.
		$wpdb->delete( $table, [ 'post_id' => $post_id ], [ '%d' ] );

		if ( empty( $links ) ) {
			\InterLinkGenius\Data\Table_Extension::update_internal_meta_counts( $post_id );
		 ...
		 */
		$should_queue = apply_filters( 'interlink_genius/queue_links_on_save', true, $post_id, $link_data_for_queue );
		...
		if ( $should_queue && Utils::is_manual_post_save() && ! empty( $link_data_for_queue ) ) {
			...
			$option_key = 'ig_pending_queue_' . $post_id;
			update_option( $option_key, $link_data_for_queue, false );

			// Schedule async processing if not already scheduled.
			if ( ! wp_next_scheduled( 'interlink_genius_queue_pending_links' ) ) {
				wp_schedule_single_event( time() + 60, 'interlink_genius_queue_pending_links' );
			}
		}
