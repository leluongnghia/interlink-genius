...
class Related_Posts {
	...
	public function get_related_posts( $post_id, $limit = 3, $args = [] ) {
		...
		if ( ! $args['force_refresh'] && 'related_posts' === $mode ) {
			$related_posts = get_post_meta( $post_id, 'interlink_genius_related_posts', true );
	...
	private function seed_reverse_relationships( $post_id, $candidates ) {
		...
		$existing_meta = Batch_Helper::batch_fetch_postmeta( $candidates, 'interlink_genius_related_posts' );
		...
		if ( ! empty( $posts_to_update ) ) {
			$values   = [];
			$meta_key = 'interlink_genius_related_posts';
