 ...
 * @since      1.0.264
 * @package    RankMathPro
 * @subpackage InterLinkGenius\Background
...
class Processor_Utils {
	...
	public static function fetch_rollback_snapshots( $rollback_batch_id ) {
		...
		if ( empty( $snapshots ) ) {
			return new WP_Error(
				'no_snapshots_found',
				__( 'No rollback data found for this batch.', 'interlink-genius' ),
				[ 'status' => 404 ]
			);
		}
		...
		if ( ! empty( $existing_progress ) && 'processing' === ( $existing_progress['status'] ?? '' ) ) {
			return new WP_Error(
				...
				sprintf(
					...
					__( 'A %s operation is already in progress. Please wait for it to complete first.', 'interlink-genius' ),
					$operation_name
				),
				[ 'status' => 409 ]
			);
