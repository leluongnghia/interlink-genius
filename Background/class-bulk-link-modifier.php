...
namespace InterLinkGenius\Background;
...
use InterLinkGenius\Services\Link_Processor;
...
class Bulk_Link_Modifier extends \WP_Background_Process {
	...
	public function start( $operation, $params = [] ) {
		...
		if ( ! in_array( $operation, [ 'delete', 'mark_safe', 'restore' ], true ) ) {
			return new WP_Error(
				'invalid_operation',
				__( 'Invalid operation type.', 'interlink-genius' ),
				[ 'status' => 400 ]
			);
		}
	...
	public function cancel() {
		...
		if ( ! empty( $batch_id ) ) {
			$wpdb->update(
				"{$wpdb->prefix}interlink_genius_history",
				[
					'status'        => 'failed',
					'error_message' => __( 'Cancelled by user', 'interlink-genius' ),
	...
	protected function complete() {
		...
		if ( 'delete' === $operation && ! empty( $batch_id ) ) {
			...
			$wpdb->delete(
				"{$wpdb->prefix}interlink_genius_snapshots",
				[ 'batch_id' => $batch_id ],
				[ '%s' ]
			);
		}
	...
	private function start_delete( $params ) {
		 ...
		 */
		$fetch_chunk_size = (int) apply_filters( 'interlink_genius/bulk_delete_fetch_chunk_size', 1000 );
		 ...
		 */
		$process_chunk_size = (int) apply_filters( 'interlink_genius/bulk_delete_chunk_size', 10 );
	...
	private function start_mark_safe( $params ) {
		 ...
		 * @param int $fetch_chunk_size Number of links per fetch. Default 1000.
		 */
		$fetch_chunk_size = (int) apply_filters( 'interlink_genius/bulk_mark_safe_fetch_chunk_size', 1000 );
		 ...
		 */
		$process_chunk_size = (int) apply_filters( 'interlink_genius/bulk_mark_safe_chunk_size', 50 );
	...
	private function start_restore( $params ) {
		...
		if ( empty( $batch_id ) ) {
			return new WP_Error(
				'missing_batch_id',
				__( 'Batch ID is required for restore operation.', 'interlink-genius' ),
				[ 'status' => 400 ]
			);
		}
		...
		$batch = DB::get_row(
			$wpdb->prepare(
				"SELECT id, source_type, status, affected_posts_count FROM {$wpdb->prefix}interlink_genius_history WHERE batch_id = %s",
				$batch_id
			)
		);
		...
		if ( ! $batch ) {
			return new WP_Error(
				'batch_not_found',
				__( 'Batch not found.', 'interlink-genius' ),
				[ 'status' => 404 ]
			);
		}
		...
		if ( 'delete' !== $batch->source_type ) {
			return new WP_Error(
				'invalid_batch_type',
				__( 'This batch is not a delete operation.', 'interlink-genius' ),
				[ 'status' => 400 ]
			);
		}
		...
		if ( 'completed' !== $batch->status ) {
			return new WP_Error(
				...
				__( 'Only completed delete operations can be restored.', 'interlink-genius' ),
				[ 'status' => 400 ]
			);
		}
		...
		$post_ids = DB::get_col(
			$wpdb->prepare(
				"SELECT post_id FROM {$wpdb->prefix}interlink_genius_snapshots WHERE batch_id = %s",
				$batch_id
			)
		);
		...
		if ( empty( $post_ids ) ) {
			return new WP_Error(
				'no_snapshots_found',
				__( 'No snapshots found for this batch. Restore not possible.', 'interlink-genius' ),
				[ 'status' => 404 ]
			);
		}
		 ...
		 */
		$chunk_size = (int) apply_filters( 'interlink_genius/bulk_restore_chunk_size', 10 );
	...
	private function batch_update_snapshots( $batch_id, $snapshots_to_update ) {
		...
		$wpdb->query(
			$wpdb->prepare(
				"UPDATE {$wpdb->prefix}interlink_genius_snapshots
