...
class Regenerate_Links extends \WP_Background_Process {
	...
	public function start() {
		 ...
		 */
		$chunk_size = (int) apply_filters( 'interlink_genius/regenerate_chunk_size', 200 );
	...
	protected function complete() {
		...
		if ( ! wp_next_scheduled( 'interlink_genius_start_crawler' ) ) {
			wp_schedule_single_event( time(), 'interlink_genius_start_crawler' );
		}
	}
	...
	private function truncate_tables() {
		...
		if ( DB::check_table_exists( 'interlink_genius_audit' ) ) {
			DB::query( "TRUNCATE TABLE {$wpdb->prefix}interlink_genius_audit" );
		}

		// Truncate parent tables.
		DB::query( "TRUNCATE TABLE {$wpdb->prefix}interlink_genius_links" );
		DB::query( "TRUNCATE TABLE {$wpdb->prefix}interlink_genius_internal_meta" );

		// Re-enable foreign key checks.
		DB::query( 'SET FOREIGN_KEY_CHECKS = 1' );

		// Delete all processed meta keys.
		delete_post_meta_by_key( 'interlink_genius_links_processed' );
