...
class Export_Processor extends \WP_Background_Process {
	...
	public static function get_export_limit() {
		 ...
		 */
		return apply_filters( 'interlink_genius/export_limit', 1000 );
	}
	...
	public function start( $type, $format, $filters, $columns, $total_count ) {
		...
		Helper::add_notification(
			sprintf(
				...
				__( 'Export started for %s records. It might take a few minutes to complete.', 'interlink-genius' ),
		 ...
		 */
		$chunk_size = apply_filters( 'interlink_genius/export_chunk_size', 5000 );
		...
		return [
			'success' => true,
			'job_id'  => $job_id,
			'message' => __( 'Export scheduled. You will receive an email when it\'s ready.', 'interlink-genius' ),
		];
	}
		...
		foreach ( $jobs as $option_name ) {
			...
			Helper::add_notification(
				sprintf(
					...
					__( 'Export completed: %1$s records. %2$s', 'interlink-genius' ),
					number_format_i18n( $job['processed'] ),
					'<a href="' . esc_url( $job['file_url'] ) . '" target="_blank">' . __( 'Download', 'interlink-genius' ) . '</a>'
				),
				[
					'type'    => 'success',
	...
	private static function send_export_email( $job ) {
		$to      = $job['user_email'];
		$subject = __( 'Rank Math: Your export is ready', 'interlink-genius' );
		...
		$message = sprintf(
			...
			__( 'Your Link Genius export is ready for download.%1$s%1$sTotal records: %2$d%1$s%1$sDownload link: %3$s%1$s%1$sNote: This link will expire in 24 hours.', 'interlink-genius' ),
			"\n",
			$job['total_count'],
			$job['file_url']
		);
