...
use InterLinkGenius\Compatibility\Helpers\DB as DB_Helper;
use InterLinkGenius\Data\Query_Builder;
use InterLinkGenius\Services\Robots_Checker;
...
class Link_Status_Crawler extends \WP_Background_Process {
	...
	public function start( $args = [] ) {
		...
		delete_option( 'interlink_genius_audit_crawler_completed' );
	...
	public static function queue_links_batch( $links_data ) {
		...
		if ( ! wp_next_scheduled( 'interlink_genius_dispatch_crawler' ) ) {
			wp_schedule_single_event( time() + 5, 'interlink_genius_dispatch_crawler' );
		}
	}
	 ...
	 *
	 * This is the WP Cron event handler for 'interlink_genius_queue_pending_links'.
	...
	public static function process_pending_link_queues() {
		...
		$pending = $wpdb->get_col(
			"SELECT option_name FROM {$wpdb->options}
			 WHERE option_name LIKE 'ig_pending_queue_%'
			 LIMIT 100"
		);
	...
	public static function clear_completion() {
		Helper::remove_notification( 'interlink_genius_audit_crawler_complete' );
	...
	public function get_queue_count() {
		...
		// wp_interlink_genius_audit_crawler_batch_{batch_id}.
		$table  = $wpdb->options;
		$column = 'option_name';
	...
	protected function task( $link ) {
		try {
			...
			$failed = (int) get_option( 'interlink_genius_audit_crawler_failed', 0 );
			update_option( 'interlink_genius_audit_crawler_failed', $failed + 1, false );
	...
	protected function get_state_option_name() {
		return 'interlink_genius_audit_crawler_state';
	}
	...
	private function get_request_options( $sslverify = true ) {
		/**
		 * Filter: 'interlink_genius/crawler_request_timeout' - HTTP timeout per URL check.
		 *
		 * @param int $timeout Timeout in seconds.
		 */
		$timeout = (int) apply_filters( 'interlink_genius/crawler_request_timeout', 5 );
	...
	private function save_status( $link, $status ) {
		global $wpdb;
		$table = $wpdb->prefix . 'interlink_genius_audit';

		// Get all link IDs with same url_hash for deduplication.
		$link_ids = $wpdb->get_col(
			$wpdb->prepare(
				"SELECT id FROM `{$wpdb->prefix}interlink_genius_links` WHERE url_hash = %s",
				$link['url_hash']
			)
		);
