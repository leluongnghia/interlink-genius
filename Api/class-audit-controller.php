...
class Audit_Controller extends Base_Controller {
	...
	public function register_routes() {
		...
		register_rest_route(
			...
			[
				...
				'args'                => [
					'filters' => [
						'description' => esc_html__( 'Filters to apply (is_internal, unchecked_only, etc.).', 'interlink-genius' ),
						'type'        => 'object',
						'required'    => false,
					],
				],
		...
		register_rest_route(
			...
			[
				...
				'args'                => [
					'link_ids' => [
						'description' => esc_html__( 'Array of link IDs to recheck.', 'interlink-genius' ),
						'type'        => 'array',
						'required'    => false,
					],
					'filters'  => [
						'description' => esc_html__( 'Filter criteria for selecting links to recheck.', 'interlink-genius' ),
						'type'        => 'object',
						'required'    => false,
					],
				],
		...
		register_rest_route(
			...
			[
				...
				'args'                => [
					'link_ids' => [
						'description' => esc_html__( 'Link IDs to mark as safe.', 'interlink-genius' ),
						'type'        => 'array',
						'required'    => false,
					],
					'reason'   => [
						'description' => esc_html__( 'Reason for marking as safe.', 'interlink-genius' ),
						'type'        => 'string',
						'required'    => false,
					],
					'filters'  => [
						'description' => esc_html__( 'Filter criteria for selecting links to mark as safe.', 'interlink-genius' ),
						'type'        => 'object',
						'required'    => false,
					],
				],
	...
	public function start_audit( $request ) {
		...
		if ( $crawler->is_active() ) {
			return $this->error(
				...
				__( 'An audit is already in progress.', 'interlink-genius' ),
				409
			);
		}
		...
		return new WP_REST_Response(
			[
				'success' => true,
				'message' => __( 'Audit started successfully.', 'interlink-genius' ),
	...
	public function recheck_links( $request ) {
		...
		if ( empty( $link_ids ) && empty( $filters ) ) {
			return $this->error(
				'invalid_params',
				__( 'Either link_ids or filters must be provided.', 'interlink-genius' ),
				400
			);
		}
		...
		if ( ! empty( $filters ) ) {
			...
			return $this->error(
				'invalid_link_ids',
				__( 'Invalid link IDs provided.', 'interlink-genius' ),
				400
			);
		}
		...
		if ( empty( $links ) ) {
			return $this->error(
				'links_not_found',
				__( 'No links found with the provided parameters.', 'interlink-genius' ),
				404
			);
		}
		...
		if ( $is_bulk ) {
			...
			if ( ! wp_next_scheduled( 'interlink_genius_dispatch_crawler' ) ) {
				wp_schedule_single_event( time() + 2, 'interlink_genius_dispatch_crawler' );
			}
		...
		if ( $check_succeeded ) {
			...
			return new WP_REST_Response(
				[
					'success' => true,
					'message' => __( 'Link rechecked successfully.', 'interlink-genius' ),
				],
				200
			);
		}
		...
		return new WP_REST_Response(
			[
				'success' => false,
				'message' => __( 'Failed to recheck link. The URL may be unreachable.', 'interlink-genius' ),
			],
			200
		);
	}
	...
	public function mark_link_safe( $request ) {
		...
		if ( $is_bulk ) {
			...
			if ( $processor->is_active() ) {
				return $this->error(
					...
					__( 'A bulk mark safe operation is already in progress.', 'interlink-genius' ),
					409
				);
			}
			...
			$processor->start(
				...
				[
					'filters'        => $filters,
					'link_ids'       => $link_ids,
					'reason'         => ! empty( $reason ) ? $reason : __( 'Bulk marked as safe', 'interlink-genius' ),
			...
			return rest_ensure_response(
				[
					'success' => true,
					'queued'  => true,
					'message' => __( 'Bulk mark safe queued for background processing.', 'interlink-genius' ),
				]
			);
		}
		...
		if ( $updated_count > 0 ) {
			$message = $is_marked_safe
				? __( 'Link marked as not broken successfully.', 'interlink-genius' )
				: __( 'Link status reverted successfully.', 'interlink-genius' );
			...
			return $this->error(
				'update_failed',
				__( 'No status record found for this link.', 'interlink-genius' ),
				404
			);
		}
