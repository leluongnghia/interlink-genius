...
use InterLinkGenius\Features\BulkUpdate\Processor as Bulk_Update_Processor;
use InterLinkGenius\Features\BulkUpdate\Preview_Processor;
use InterLinkGenius\Features\KeywordMaps\Keyword_Map_Processor;
use InterLinkGenius\Features\KeywordMaps\Preview_Processor as KM_Preview_Processor;
...
class Operations_Rest extends Base_Controller {
	...
	public function register_routes() {
		...
		register_rest_route(
			...
			[
				...
				'args'                => [
					'operation' => [
						'description' => esc_html__( 'Operation type.', 'interlink-genius' ),
						'type'        => 'string',
						'required'    => true,
						'enum'        => array_keys( self::OPERATIONS ),
					],
					'id'        => [
						'description' => esc_html__( 'Optional operation ID (for keyword maps).', 'interlink-genius' ),
						'type'        => 'integer',
					],
				],
			]
		...
		register_rest_route(
			...
			[
				...
				'args'                => [
					'operation' => [
						'description' => esc_html__( 'Operation type to cancel.', 'interlink-genius' ),
						'type'        => 'string',
						'required'    => true,
						'enum'        => array_keys( self::OPERATIONS ),
					],
		...
		register_rest_route(
			...
			[
				...
				'args'                => [
					'operation' => [
						'description' => esc_html__( 'Operation type to clear.', 'interlink-genius' ),
						'type'        => 'string',
						'required'    => true,
						'enum'        => array_keys( self::OPERATIONS ),
					],
			...
			return new WP_REST_Response(
				[
					...
					'message' => sprintf(
						...
						__( 'No %s operation in progress.', 'interlink-genius' ),
	...
	private function perform_cancel( $operation, $id ) {
		...
		if ( 'keyword_map_preview' === $operation ) {
			if ( ! $id ) {
				return new WP_Error(
					'missing_id',
					__( 'Keyword map ID is required for preview cancellation.', 'interlink-genius' ),
					[ 'status' => 400 ]
				);
			}
		...
		if ( ! method_exists( $processor_class, 'get' ) ) {
			return new WP_Error(
				'invalid_processor',
				__( 'Invalid processor for this operation.', 'interlink-genius' ),
				[ 'status' => 500 ]
			);
		}
	...
	private function perform_clear( $operation, $id ) {
		...
		if ( 'keyword_map_preview' === $operation ) {
			if ( ! $id ) {
				return new WP_Error(
					'missing_id',
					__( 'Keyword map ID is required.', 'interlink-genius' ),
					[ 'status' => 400 ]
				);
			}
			KM_Preview_Processor::get()->cancel( $id );
		...
		if ( ! method_exists( $processor_class, 'clear_completion' ) ) {
			return new WP_Error(
				'not_supported',
				__( 'Clear operation not supported for this operation type.', 'interlink-genius' ),
				[ 'status' => 400 ]
			);
		}
	...
	private function get_operation_label( $operation ) {
		$labels = [
			'regenerate'          => __( 'link regeneration', 'interlink-genius' ),
			'audit'               => __( 'link audit', 'interlink-genius' ),
			'bulk_delete'         => __( 'bulk delete', 'interlink-genius' ),
			'bulk_mark_safe'      => __( 'bulk mark safe', 'interlink-genius' ),
			'bulk_restore'        => __( 'bulk restore', 'interlink-genius' ),
			'bulk_update_preview' => __( 'bulk update preview', 'interlink-genius' ),
			'bulk_update_apply'   => __( 'bulk update', 'interlink-genius' ),
			'keyword_map_preview' => __( 'keyword map preview', 'interlink-genius' ),
			'keyword_map_apply'   => __( 'keyword map execution', 'interlink-genius' ),
			'export'              => __( 'export', 'interlink-genius' ),
		];
	...
	private function get_cancel_message( $operation ) {
		$messages = [
			'regenerate'          => __( 'Link regeneration cancelled successfully.', 'interlink-genius' ),
			'audit'               => __( 'Link audit cancelled successfully.', 'interlink-genius' ),
			'bulk_delete'         => __( 'Bulk delete operation cancelled successfully.', 'interlink-genius' ),
			'bulk_mark_safe'      => __( 'Bulk mark safe operation cancelled successfully.', 'interlink-genius' ),
			'bulk_restore'        => __( 'Bulk restore operation cancelled successfully.', 'interlink-genius' ),
			'bulk_update_preview' => __( 'Bulk update preview cancelled successfully.', 'interlink-genius' ),
			'bulk_update_apply'   => __( 'Bulk update cancelled successfully.', 'interlink-genius' ),
			'keyword_map_preview' => __( 'Keyword map preview cancelled successfully.', 'interlink-genius' ),
			'keyword_map_apply'   => __( 'Keyword map execution cancelled successfully.', 'interlink-genius' ),
			'export'              => __( 'Export cancelled successfully.', 'interlink-genius' ),
		];

		return $messages[ $operation ] ?? __( 'Operation cancelled successfully.', 'interlink-genius' );
	}
	...
	private function get_clear_message( $operation ) {
		$messages = [
			'regenerate'          => __( 'Link regeneration completion data cleared.', 'interlink-genius' ),
			'audit'               => __( 'Link audit completion data cleared.', 'interlink-genius' ),
			'bulk_delete'         => __( 'Bulk delete completion data cleared.', 'interlink-genius' ),
			'bulk_mark_safe'      => __( 'Bulk mark safe completion data cleared.', 'interlink-genius' ),
			'bulk_restore'        => __( 'Bulk restore completion data cleared.', 'interlink-genius' ),
			'bulk_update_preview' => __( 'Bulk update preview data cleared.', 'interlink-genius' ),
			'bulk_update_apply'   => __( 'Bulk update completion data cleared.', 'interlink-genius' ),
			'keyword_map_preview' => __( 'Keyword map preview data cleared.', 'interlink-genius' ),
			'keyword_map_apply'   => __( 'Keyword map execution completion data cleared.', 'interlink-genius' ),
			'export'              => __( 'Export completion data cleared.', 'interlink-genius' ),
		];

		return $messages[ $operation ] ?? __( 'Operation completion data cleared.', 'interlink-genius' );
	}
}
