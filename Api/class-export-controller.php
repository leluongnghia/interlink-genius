...
class Export_Controller extends Base_Controller {
	...
	public function register_routes() {
		...
		register_rest_route(
			...
			[
				...
				'args'                => [
					'type'    => [
						'description' => esc_html__( 'Export type (links or posts).', 'interlink-genius' ),
						'type'        => 'string',
						'required'    => true,
					],
					'format'  => [
						'description' => esc_html__( 'Export format (csv, json, excel).', 'interlink-genius' ),
						'type'        => 'string',
						'required'    => true,
					],
					'filters' => [
						'description' => esc_html__( 'Filters to apply to the export.', 'interlink-genius' ),
						'type'        => 'object',
						'required'    => false,
					],
					'columns' => [
						'description' => esc_html__( 'Column definitions.', 'interlink-genius' ),
						'type'        => 'array',
						'required'    => true,
					],
					'total'   => [
						'description' => esc_html__( 'Total number of records to export.', 'interlink-genius' ),
						'type'        => 'integer',
						'required'    => true,
					],
				],
	...
	public function schedule_export( $request ) {
		...
		if ( ! in_array( $type, [ 'links', 'posts' ], true ) ) {
			return $this->error( 'invalid_type', __( 'Invalid export type.', 'interlink-genius' ), 400 );
		}

		// Validate format.
		if ( ! in_array( $format, [ 'csv', 'json', 'excel' ], true ) ) {
			return $this->error( 'invalid_format', __( 'Invalid export format.', 'interlink-genius' ), 400 );
		}
