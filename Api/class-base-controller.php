...
abstract class Base_Controller extends WP_REST_Controller {
	...
	public function check_admin_permission( $request ) {
		if ( ! Helper::has_cap( 'link_builder' ) ) {
			return new WP_Error(
				'rest_forbidden',
				__( 'Sorry, you are not allowed to access this resource.', 'interlink-genius' ),
	...
	public function check_post_edit_permission( $request ) {
		...
		if ( $post_id <= 0 ) {
			return new WP_Error(
				'rest_invalid_param',
				__( 'Invalid post ID.', 'interlink-genius' ),
				[ 'status' => 400 ]
			);
		}
		...
		if ( ! current_user_can( 'edit_post', $post_id ) ) {
			return new WP_Error(
				'rest_forbidden',
				__( 'Sorry, you are not allowed to edit this post.', 'interlink-genius' ),
				[ 'status' => rest_authorization_required_code() ]
			);
		}
		return true;
	...
	protected function check_content_ai_requirements() {
		if ( \InterLinkGenius\Compatibility\Helper::is_module_active( 'content-ai' ) ) {
			return true;
		}

		return new WP_Error(
			'content_ai_module_disabled',
			__( 'Content AI module is not enabled. Please enable it to use this feature.', 'interlink-genius' ),
			[
				'status'      => 403,
