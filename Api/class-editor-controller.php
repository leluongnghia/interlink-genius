...
use InterLinkGenius\Services\Related_Posts;
use InterLinkGenius\Services\Utils;
use InterLinkGenius\Services\Content_Similarity;
...
class Editor_Controller extends Base_Controller {
	...
	public function register_routes() {
		...
		register_rest_route(
			...
			[
				...
				'args'                => [
					'post_id' => [
						'description' => esc_html__( 'Post ID.', 'interlink-genius' ),
						'type'        => 'integer',
						'required'    => true,
					],
					'text'    => [
						'description' => esc_html__( 'Selected text used for suggestions.', 'interlink-genius' ),
						'type'        => 'string',
						'required'    => false,
					],
					'current' => [
						'description' => esc_html__( 'Current editor state summary.', 'interlink-genius' ),
						'type'        => 'object',
						'required'    => false,
					],
				],
	...
	public function link_opportunities( WP_REST_Request $request ) {
		...
		update_post_meta( $post_id, 'interlink_genius_ai_link_suggestions', $items );
	...
	public function related_posts( WP_REST_Request $request ) {
		...
		update_post_meta( $post_id, 'interlink_genius_related_posts', array_map( 'absint', (array) $ids ) );
